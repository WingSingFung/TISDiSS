#!/usr/bin/env bash
export PYTHONPATH="/23TW026/mss/TISDiSS:$PYTHONPATH"
...
if ! "${skip_eval}"; then
    ...
    if [ ${stage} -le 8 ] && [ ${stop_stage} -ge 8 ]; then
        ...
        for score_obs in true false; do
            ...
            # if [ -n "${inference_enh_config}" ]; then
            #     results_md="RESULTS_$(basename ${inference_enh_config}).md"
            # else
            #     results_md="RESULTS.md"
            # fi
            results_md="RESULTS.md"
            ./scripts/utils/show_enh_score.sh "${_dir}/../.." > "${_dir}/../../${results_md}"
        done
        log "Evaluation result for observation: ${data_feats}/RESULTS.md"
        log "Evaluation result for enhancement: ${enh_exp}/${results_md}"
