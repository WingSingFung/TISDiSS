...
class ESPnetEnhancementModel(AbsESPnetModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.normalize_variance_per_ch and speech_pre is not None:
            ...
            if others.get("aux_speech_pre") is not None:
                others["aux_speech_pre"] = [
                    [sp * mix_std_ for sp in aux_speech_pre]
    ...
    def forward_enhance(
        ...
        if feature_pre is not None:
            ...
            if pre_is_multi_list:
                ...
                if others.get("aux_speech_pre") is not None:
                    others["aux_speech_pre"] = [
                        [self.decoder(ps, speech_lengths, fs=fs)[0] for ps in aux_speech_pre]
                        for aux_speech_pre in others["aux_speech_pre"]
                    ]
        else:
    ...
    def forward_loss(
        ...
        for loss_wrapper in self.loss_wrappers:
            ...
            if isinstance(criterion, TimeDomainLoss):
                ...
                if others.get("aux_speech_pre") is not None:
                    ...
                    for aux_speech_pre in others["aux_speech_pre"]:
                        ...
                        aux_l, aux_s, aux_o = loss_wrapper(sref, spre, {**others, **o})
            ...
            if others.get("aux_speech_pre") is not None:
                stats[f"{criterion.name}_mainloss"] = l.clone().detach()
                # register aux loss value
                for aux_i in range(len(aux_l_list)):
                    stats[f"{criterion.name}_aux{aux_i}loss"] = aux_l_list[aux_i].clone().detach()
                l = ((l + sum(aux_l_list)) / (len(aux_l_list) + 1))
