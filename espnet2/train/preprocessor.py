...
class DynamicMixingPreprocessor(AbsPreprocessor):
    def __init__(
        ...
        self.fix_speech_length = fix_speech_length
    ...
    def _read_source_(self, key, speech_length):
        ...
        if speech_length > source.shape[0]:
            ...
            start = random.randint(0, source.shape[0] - speech_length)
            source = source[start : start + speech_length]
    ...
    def _mix_speech_(self, uid, data):
        ...
        if self.fix_speech_length is not None:
            if speech_length < self.fix_speech_length:
                pad = self.fix_speech_length - speech_length
                data[self.mixture_source_name] = np.pad(data[self.mixture_source_name], (0, pad), "reflect")
            else:
                start = random.randint(0, speech_length - self.fix_speech_length)
                data[self.mixture_source_name] = data[self.mixture_source_name][start : start + self.fix_speech_length]

            speech_length = self.fix_speech_length
